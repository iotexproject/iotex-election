...
func (t *tableDB) NewBucketTable() error {
	_, err := t.db.Exec("CREATE TABLE IF NOT EXISTS buckets (id INTEGER PRIMARY KEY AUTOINCREMENT, hash BLOB, startTime BLOB, duration TEXT, amount BLOB, decay INTEGER, voter BLOB, candidate BLOB, bucketIndex INTEGER)")
	return err
}
...
func (t *tableDB) NewHeightToRegTable() error {
	_, err := t.db.Exec("CREATE TABLE IF NOT EXISTS heightToReg (height INTEGER, index INTEGER)")
	return err
}
//NewHeightToBucketTable creates heightToBucket table
func (t *tableDB) NewHeightToBucketTable() error {
	_, err := t.db.Exec("CREATE TABLE IF NOT EXISTS heightToBucket (height INTEGER, index INTEGER)")
	return err
}
//NewHeightToTimeTable creates heightToTime table
func (t *tableDB) NewHeightToTimeTable() error {
	_, err := t.db.Exec("CREATE TABLE IF NOT EXISTS heightToTime (height INTEGER, time BLOB)")
	return err
}
